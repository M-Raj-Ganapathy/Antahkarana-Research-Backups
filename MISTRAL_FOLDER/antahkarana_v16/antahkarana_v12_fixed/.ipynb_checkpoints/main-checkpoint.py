...
def _build_report(summary: Dict, ablation_summary: Dict, sig_results: Dict) -> str:
    ...
    lines.append("=" * 78)
    # lines.append(f"  Model : Meta-Llama-3.1-8B-Instruct")
    lines.append(f"  Model : Mistral-7B-Instruct-v0.3")
    ...
    header  = f"{'Method':<22}" + "".join(f"{'EM':>8}{'F1':>8}" for _ in range(len(summary)))
    ds_row  = " " * 22 + "".join(
        f"{ds[:8]:>8}{'' :>8}" for ds in summary
    )

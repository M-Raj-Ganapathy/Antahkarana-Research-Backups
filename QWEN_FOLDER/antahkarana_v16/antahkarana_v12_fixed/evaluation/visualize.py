...
def plot_ablation(
    ...
    """
    ablation_summary: {config_name: aggregated_scores_dict}
    """
    if not MPL_AVAILABLE:
        return None

    configs = list(ablation_summary.keys())
    means   = [_get_mean(ablation_summary[c], metric) for c in configs]
    errs    = [_get_ci(ablation_summary[c], metric)   for c in configs]
    ...
    # Color: full model red, others grey
    ...
    ax.set_title(f"Ablation Study — {metric.upper()} on HotpotQA (n=50)")
    ax.set_xlim(0, max(means) * 1.25 if means else 1.0)
